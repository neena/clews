...
(function() {
  this.reloadFormAs = function(patientID) {
    return $('#main').load(document.URL + " #content", "patient_id=" + patientID);
  };

}).call(this);
(function() {
  $(document).on('ready page:load', function() {
    return $('.reminder').click(function() {
      return $(this).children(".description").collapse('toggle');
    });
  });
...
(function() {
  $(document).on('ready page:load', function() {
    ...
    loadChart = function(chart) {
      return $('#' + chart.id).highcharts({
        chart: {
          renderTo: chart.id,
          type: chart.type === "bp" ? 'column' : void 0
        },
        title: {
          text: chart.title
        },
        yAxis: {
          title: {
            text: chart.units ? chart.title + " (" + chart.units + ")" : chart.title
          },
        ...
        tooltip: {
          formatter: chart.type === "bp" ? function() {
            return '<b>' + new Date(this.x).toLocaleString('en-GB') + '</b> <br> Systolic: ' + this.y + 'mmHg <br> Diastolic: ' + this.point.low + 'mmHg ';
          } : function() {
            return '<b>' + new Date(this.x).toLocaleString('en-GB') + '</b> <br>' + this.series.chart.title.text + ': ' + this.y + this.series.name;
          }
        }
      });
    };
    Highcharts.setOptions({
      ...
      tooltip: {
        formatter: function() {
          return '<b>' + new Date(this.x).toLocaleString('en-GB') + '</b> <br>' + this.y;
        }
      }
    });
